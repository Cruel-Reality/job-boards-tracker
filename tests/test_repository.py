...
def test_upsert_sets_company_id(clean_db):
    ...
    assert get_jobs()[0].company_id == company.id
...
def test_get_jobs_filter_by_application_status_and_tracked(clean_db):
    ...
    job = get_jobs()[0]
    add_application(
        JobApplicationCreate(job_posting_id=job.id, status=JobStatusEnum.applied)
    )

    assert len(get_jobs(application_status=JobStatusEnum.applied)) == 1
    assert len(get_jobs(application_status=JobStatusEnum.rejected)) == 0
    assert len(get_jobs(tracked=True)) == 1
    assert len(get_jobs(tracked=False)) == 0
...
def test_get_jobs_filter_by_size_and_sector(clean_db):
    ...
    assert len(get_jobs(size=SizeEnum.big)) == 1
    assert len(get_jobs(size=SizeEnum.startup)) == 0
    assert len(get_jobs(sector=SectorEnum.finance)) == 1
    assert len(get_jobs(sector=SectorEnum.tech)) == 0
