...
def test_get_jobs_passes_query_params():
    with patch("app.main.get_jobs", return_value=([], 0)) as mock_get_jobs:
        client.get(
            ...
            "&category=data&seniority=senior&location=york&remote=true&search=engineer"
        )
...
def test_get_jobs_rejects_invalid_sector():
    response = client.get("/jobs?sector=banana")
    assert response.status_code == 422
